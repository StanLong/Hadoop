...
def get_url(server_name, listen_port,topic_name):
    ...
    URL = "http://"+server_name+":" + str(listen_port)+"/jmx?qry=Hadoop:"+topic_name
    print(URL)
    return URL
...
def json_processing(server_name, listen_port):
    ...
    yarn_dict = {}
    app_json = load_url_as_dictionary(
        get_url(server_name, listen_port, "service=ResourceManager,name=QueueMetrics,q0=root"))

    yarn_dict['apps-submitted'] = app_json['beans'][0]['AppsSubmitted']
    yarn_dict['apps-pending'] = app_json['beans'][0]['AppsPending']
    yarn_dict['apps-running'] = app_json['beans'][0]['AppsRunning']
    yarn_dict['apps-completed'] = app_json['beans'][0]['AppsCompleted']
    yarn_dict['apps-failed'] = app_json['beans'][0]['AppsFailed']
    yarn_dict['apps-killed'] = app_json['beans'][0]['AppsKilled']

    nodes_json = load_url_as_dictionary(
        get_url(server_name, listen_port, "service=ResourceManager,name=ClusterMetrics"))
    yarn_dict['activenodes'] = nodes_json['beans'][0]['NumActiveNMs']
    return yarn_dict
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 5:
        ...
        yarn_hostname, yarn_listen_port)
...
def json_processing(server_name, listen_port, monitor_type):
    ...
    if "DFS" == monitor_type:
        ...
        # NameNodeInfo
        namenode_dict['files_and_directorys'] = fsns_json['beans'][0]['FilesTotal']
        namenode_dict['blocks'] = nninfo_json['beans'][0]['TotalBlocks']
        namenode_dict['configured_capacity'] = nninfo_json['beans'][0]['Total']/ 1073741824
        namenode_dict['dfs_used'] = nninfo_json['beans'][0]['Used'] / 1073741824
        namenode_dict['dfs_used_persent'] = min_value_display(
            float(nninfo_json['beans'][0]['PercentUsed']))
        namenode_dict['non_dfs_used'] = nninfo_json['beans'][0]['NonDfsUsedSpace'] / 1073741824
        namenode_dict['dfs_remaining'] = nninfo_json['beans'][0]['Free']/ 1073741824
